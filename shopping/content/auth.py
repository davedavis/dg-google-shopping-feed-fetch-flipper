...
def authorize(config):
    ...
    if "path" not in config:
        ...
        sys.exit(1)
    service_account_path = os.path.join(config["path"], _constants.SERVICE_ACCOUNT_FILE)
    client_secrets_path = os.path.join(config["path"], _constants.CLIENT_SECRETS_FILE)
